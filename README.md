...
| PDF to Images Automatically | Drop a PDF, and your browser converts every page to an image instantly (no server processing). |
...
**Important:** The `.env` file is **not** included in this repository – you must create it yourself.  
Do **not** upload or commit this file to any public place – it contains your secret API key.

- Open your favourite text editor (Notepad, VS Code, etc.).
- Create a new file named exactly `.env` (no file extension – just the name `.env`).
- Inside, paste the following line, replacing `your-api-key-here` with your actual key:

```
OPENROUTER_API_KEY=your-api-key-here
```

For example, if your key is `sk-or-v1-abcd1234...`, the file should contain:

```
OPENROUTER_API_KEY=sk-or-v1-abcd1234...
```

- **Save this file in the same folder as `main.py`** (the root of the project).

> **Pro tip:** On Windows, if the system won't let you name a file just `.env`, use `touch .env` in PowerShell or Command Prompt, or use VS Code – it handles dot-files without complaining.
...
**Option A – Click the paperclip button**:
...
- Drag a PDF onto the page, or use the paperclip button.
...
- Click the moon/sun button at the top right.
...
├── README.md           # This file
└── .env                # Your API key (create this yourself – see step 4)
```
...
- NVIDIA – for releasing such powerful models for free.
- Meta – for the open-source models that keep the fallback chains alive.
...
**P.S.** "Acads" is short for "Academics". I wanted something that felt personal, because that's exactly what this is – your personal AI tutor.
```
...
- Removed the entire **"License"** section.
- Removed the MIT license mention from the final notes.
- Everything else stays the same – the setup guide, user manual, troubleshooting, and the personal story all remain intact.
